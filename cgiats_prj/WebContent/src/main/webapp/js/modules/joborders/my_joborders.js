;(function(angular){
	...
	angular.module("myJobOrders",['ngMaterial', 'ngMessages','ui.bootstrap','jcs-autoValidate', 'DatePicker'])
    .controller("myJobOrdersController",function($rootScope, $scope, blockUI, $http, $timeout, $filter, $mdDialog, $mdMedia, $window, $state,dateRangeService,$location){
    	...
    	$scope.getAllADMs = function(){
    		var response = $http.get("commonController/getAllADMs/"+$rootScope.rsLoginUser.userId);
    	...
    	$scope.onload = function(){
			...
			if($location.search() && $location.search().interval){
...
*/			 if($rootScope.rsLoginUser.userRole == constants.DM){
		...
		$scope.viewADMJobOrders = function(selectAdmId){
			candidateData = [];
			$scope.recordsBelongsTo = "";
			dispalyTable();
			$("#searchtable").css("display", "block");
			$scope.pageNumber = 1;
			if(selectAdmId){
			search_joborder(selectAdmId,false);
			}
		}
...
//			alert("json"+JSON.stringify($scope.searchFields));
			var response = $http.post("jobOrder/getMyJobOrders",$scope.searchFields);
			response.success(function(data, status,headers, config) 
					{
					...
					.forEach(function(candidate, index) {
											...
											response
													.success(function(
														...
														if($rootScope.rsLoginUser.userRole == constants.DM){
															...
															for (var i = 0; i < expansionData.length; i++) {
																tableres += '<tr><td>'
																		...
																		if($rootScope.rsLoginUser.userRole == constants.DM){
																			tableres +='<td><a href="#/offerletter/createofferletter?submittalId='+expansionData[i].submittalId+'&pageName='+constants.MYJOBORDERS+'&candidateId='+expansionData[i].candidateId+'&jobOrderId='+expansionData[i].jobOrderId+'">Create/Edit Offer Letter</a></td></tr>';
																		}else{
																			tableres +='</tr>'
																		}
																		...
																		tableres +='<script type="text/javascript">'
																		 ...
																		 +'success:function(data){'
																		 +'$("#submitaldelete").hide();'
																		 +'angular.element("#myJobOrdersController").scope().searchjoborder();'
																		 +'angular.element("#myJobOrdersController").scope().$apply() '
																		 +'}'
																		 +'});'
																		 +'}'
																		 +'else{'
														...
														tableres += '</table><script type="text/javascript">function viewResumes(id){if((document.getElementById(id.id).value)==0){ $.growl	.error({message : "Select SearchBy Field for Viewing  Matching Resumes"});} else{ window.location="#/candidates/search?paramValue="+document.getElementById(id.id).value}}</script>'
															 ...
															 +'window.location="#/submitals/createsubmitals?jobOrder="+job_orderId+"&pageName="+constants.MYJOBORDERS'
															 +'}else{'
						...
						function editJobOrder(value, row,
							...
							return [
							        ...
							        '<a class="copy actionIcons" title="Edit" href="#/joborders/editJobOrders/'+row.jobOrderId+'/'+row.dm+'/'+constants.MYJOBORDERS+'">'+row.jobOrderId+'</a> '
									 ]
									.join('');
							}else{
								return [
						...
						function actionFormatter(value, row,
							...
							var emailclass = row.hot ? "fa fa-envelope": "";
							...
							if($rootScope.rsLoginUser.userRole == constants.Recruiter){
								...
								if($rootScope.rsLoginUser.userRole == constants.DM){
									if(!(row.status === constants.CLOSED || row.status === constants.FILLED)){
									return [
									        '<a  ><img class="hotlist actionIcons" id = "hotimg'+ row.jobOrderId+ '" flex-gt-md="auto" title="'	+ hotTitle+ '" width="12" height="12" src='+ src + ' /></a>',
											'<a class="view actionIcons" title="View Details" flex-gt-md="auto"><i class="fa fa-search" style="font-size:12px;"></i></a>',
											...
											'<a class="email actionIcons" title="Resend Email" flex-gt-md="auto"><i class="'+ emailclass + '" aria-hidden="true"></i></a>',
						...
						window.operateEvents = {
							...
							/* View Resume details */
								...
								'click .email' : function(e, value,
									...
									$(".underlay").show();
									var response = $http
											.get('jobOrder/resendEmails/'+ jobOrderId);
									response
											.success(function(data,
													status,
													headers, config) {
												$(".underlay").hide();
								   	     		$.growl	.notice({message : " Mail Sent Successfully "});
											});
									response.error(function(data, status, headers, config){
					        			  if(status == constants.FORBIDDEN){
					        				location.href = 'login.html';
							...
							'click .edit' : function(e, value,
									row, index) {
								$state.transitionTo("editJobOrders",{jobOrderId : row.jobOrderId,dmName:row.dm,page:constants.MYJOBORDERS});
							...
							'click .hotlist' : function(e,value, row, index) {
								/*var hottrue = "Are you sure you want to remove Job Order from hotlist ?";
								var hotfalse = "Are you sure you want to add Job Order to hotlist?"
									var alert = row.hot ? hottrue : hotfalse;*/
								$mdDialog
										.show(
												{
													controller : HotJobDialogController,
						...
						function HotJobDialogController ($scope,
								$mdDialog, rowData){
							$scope.hide = function() {
								$mdDialog.hide();
							};
...
//									 alert (answer +"  "+rowData.jobOrderId);  
									 
									 if((rowData.status === constants.FILLED || rowData.status === constants.CLOSED)){
										 $(".underlay").hide();
										 $.growl.notice({title : "Warning!", message : rowData.jobOrderId +" is aleready "+rowData.status+" so you can't make it as hot" });
									 }else{
									 
									 var hotjoblist = {
												"candidateId" : rowData.jobOrderId,
												"reason" : answer
											}
									  var response = $http.post("jobOrder/hotComment", hotjoblist);
						   	          response.success(function(data, status, headers, config) {
						   	        	search_joborder(null,false);
						   	        	$(".underlay").hide();
						   	        	$.growl	.success({title : "Update!", message : rowData.jobOrderId +"#"+data.statusMessage });
						   	  		   });
