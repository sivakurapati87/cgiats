;(function(angular){
	...
	angular.module("allJobOrders",['ngMaterial', 'ngMessages','ui.bootstrap','jcs-autoValidate', 'DatePicker','ngJsonExportExcel'])
    .controller("allJobOrdersController",function($rootScope, $scope, blockUI, $http, $timeout, $filter, $mdDialog, $mdMedia, $window, $state,dateRangeService) {
    	$('.ranges').empty();
					...
					.forEach(function(candidate, index) {
											...
											response
													.success(function(
															...
															for (var i = 0; i < expansionData.length; i++) {
																tableres += '<tr><td>'
																		...
																		+'<script type="text/javascript">'
																		 ...
																		 +'success:function(data){'
																		 +'$("#submitaldelete").hide();'
																		 +'angular.element("#allJobOrdersController").scope().searchjoborder();'
																		 +'angular.element("#allJobOrdersController").scope().$apply() '
																		 +'}'
																		 +'});'
																		 +'}'
																		 +'else{'
														...
														tableres += '</table><script type="text/javascript">function viewResumes(id){if((document.getElementById(id.id).value)==0){ $.growl	.error({message : "Select SearchBy Field for Viewing  Matching Resumes"});} else{ window.location="#/candidates/search?paramValue="+document.getElementById(id.id).value}}</script>'
															 ...
															 +'window.location="#/submitals/createsubmitals?jobOrder="+job_orderId+"&pageName="+constants.ALLJOBORDERS'
															 +'}else{'
						...
						window.operateEvents = {
							...
							'click .edit' : function(e, value,
								...
								var url = 	$state.href("editJobOrders",{jobOrderId : row.jobOrderId,dmName:row.dm,page:constants.ALLJOBORDERS});
