;(function(angular){
		...
		angular.module("categoryjobordersreport",[])
	    .controller("categoryjoborderscontroller",function($rootScope, $scope, blockUI, $http, $timeout, $filter, $mdDialog, $mdMedia, $window, $state,dateRangeService, $stateParams) {
...
//	    		alert("SEND OBJ::"+JSON.stringify($scope.getobj));
	    		var response = $http.post("customReports/getJobOrdersByCategoryWithTime",$scope.getobj);
	    	...
	    	function dmcategoryjoborders() {
	    		
	    		$scope.dmgetobj = {
						"startDate" : $stateParams.startDate,
						"endDate" : $stateParams.endDate,
						"status" : $stateParams.status.toUpperCase(),
						"dmName" : $stateParams.dmName,
						"category" : $stateParams.category.replace("%2F","/")
						
				};
	    		
//	    		alert("SEND OBJ::"+JSON.stringify($scope.dmgetobj));
	    		var response = $http.post("customReports/getJobOrdersByCategoryAndDM",$scope.dmgetobj);
				response.success(function(data, status,headers, config) 
						{
					$scope.allJobOrdersList = data;
					candidateData = [];
